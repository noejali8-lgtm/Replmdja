...
- **App Builder**: Mobile Replit-like UI — create projects via AI chat, browse projects, account page
  ...
  - **Ensemble Tab** (7th tab): Real multi-AI collaboration via OpenRouter API. Select 2–6 free models → Diverge phase (parallel calls) → Synthesise phase (best elements merged). Live SSE streaming, phase indicator, per-model result cards. OpenRouter API key stored in localStorage. Backend: `POST /api/openrouter/ensemble` (SSE stream).
